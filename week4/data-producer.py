...
def fetch_price(symbol, producer, topic_name):
	"""
	helper function to retrieven asset data and send it to kafka
	...
	try:
		...
		payload = {'Symbol':str(symbol),
		            ...
		            'LstTradeDateTime':str(timestamp)}
...
def shutdown_hook(producer):
	...
	try:
		...
		logger.warn('Failed to flush pending messages to kafka, caused by %s', kafka_error.messages)
	finally:
		try:
			logger.info('Closing kafka connection')
			producer.close(10)
		except Exception as e:
			logger.warn('Failed to close kafka connection, caused by: %s', e.messages)
...
def check_symbol(symbol):
	...
	try:
		...
		if symbol not in product_ids:
			logger.warn('Symbol %s not supported. The list of supported sybols: %s', ssymbol, product_ids)
			exit()
	except Exception as e:
